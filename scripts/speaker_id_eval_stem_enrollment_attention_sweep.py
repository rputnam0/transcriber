...
from speaker_id_eval_stem_enrollment_extractor_sweep import (  # noqa: E402
    _balanced_indices,
    _enrollment_indices,
    _enrollment_source,
    _fixed_enrollment_indices,
    _one_hot,
)
...
class EnrollmentTokenEncoder(nn.Module):
    def __init__(self, channels: int, tokens: int) -> None:
        super().__init__()
        self.tokens = int(tokens)
        self.net = nn.Sequential(
            nn.Conv1d(1, 48, kernel_size=15, stride=4, padding=7),
            nn.GroupNorm(6, 48),
            nn.PReLU(),
            nn.Conv1d(48, 96, kernel_size=15, stride=4, padding=7),
            nn.GroupNorm(8, 96),
            nn.PReLU(),
            nn.Conv1d(96, channels, kernel_size=9, stride=2, padding=4),
    ...
    def forward(self, wave: torch.Tensor) -> torch.Tensor:
        return self.net(wave.unsqueeze(1))


class EnrollmentAttentionTasNetExtractor(nn.Module):
    def __init__(
        self,
        *,
        enc_feats: int = 128,
        ...
        self.enrollment_encoder = EnrollmentTokenEncoder(bottleneck, tokens=enrollment_tokens)
        self.type_embedding = nn.Parameter(torch.zeros(2, bottleneck))
        ...
        self.fuse = nn.Sequential(
            nn.Conv1d(bottleneck * 2, bottleneck, kernel_size=1),
            nn.GroupNorm(8, bottleneck),
            nn.PReLU(),
        )
        one_hot_dim = len(CORE_SPEAKERS) if include_one_hot else 0
        self.cond = nn.Sequential(
            nn.Linear(bottleneck * 3 + one_hot_dim, max(128, cond_dim)),
            nn.SiLU(),
            nn.Linear(max(128, cond_dim), cond_dim),
            nn.SiLU(),
        )
    ...
    def forward(
        ...
        pos_tokens = self.enrollment_encoder(positive_enrollment)
        neg_tokens = self.enrollment_encoder(negative_enrollment)
        pos_tokens = pos_tokens + self.type_embedding[0].view(1, -1, 1)
        neg_tokens = neg_tokens + self.type_embedding[1].view(1, -1, 1)
        enrollment_tokens = torch.cat([pos_tokens, neg_tokens], dim=2).transpose(1, 2)
        ...
        hidden = self.fuse(torch.cat([hidden, attended.transpose(1, 2)], dim=1))
        pos_summary = pos_tokens.mean(dim=-1)
        neg_summary = neg_tokens.mean(dim=-1)
        features = [pos_summary, neg_summary, pos_summary - neg_summary]
...
def _train_model(
    ...
    *,
    ...
    device: str,
    model_path: Path,
) -> EnrollmentAttentionTasNetExtractor:
    model = EnrollmentAttentionTasNetExtractor(
        ...
        enrollment_tokens=int(args.enrollment_tokens),
...
def _write_leave_group_embeddings(
    ...
    *,
    ...
    for group in groups:
        ...
        print(
            f"enrollment_attention_fold group={group} "
        ...
        for offset in range(0, len(test_indices), int(args.eval_batch_size)):
            ...
            print(
                f"embedded_enrollment_attention_lgo group={group} "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a direct enrollment/mix attention eval-stem target extractor."
    )
    parser.add_argument(
        "--prepared-root",
        type=Path,
    ...
    parser.add_argument(
        "--model-output",
        type=Path,
        default=Path("/tmp/codex_eval_stem_enrollment_attention_tasnet.pt"),
    )
    parser.add_argument(
        "--embedding-output",
        type=Path,
        default=Path("/tmp/codex_eval_stem_enrollment_attention_tasnet_embeddings.npz"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/tmp/codex_eval_stem_enrollment_attention_tasnet.json"),
    )
    parser.add_argument(
    ...
    parser.add_argument("--enrollment-tokens", type=int, default=16)
    ...
    payload = {
        "model": "eval_stem_enrollment_attention_tasnet",
    ...
    print(
        ",".join(
            [
                "eval_stem_enrollment_attention_tasnet/lda_shrinkage",
