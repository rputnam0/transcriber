...
from speaker_id_target_extractor_sweep import (
...
def _normalize_wave(wave: np.ndarray) -> np.ndarray:
    ...
    rms = float(np.sqrt(np.mean(wave * wave) + 1e-8))
    wave = wave / max(rms, 1e-4)
    peak = max(float(np.max(np.abs(wave))), 1e-6)
    return (wave / max(peak, 1.0)).astype(np.float32)
...
class OneHotMaskUNet(nn.Module):
    def __init__(
        self, embedding_dim: int, *, cond_channels: int = 8, base_channels: int = 32
    ) -> None:
        super().__init__()
        ...
        self.enc1 = ConvBlock(1 + cond_channels, base_channels)
    ...
    def forward(self, logmag: torch.Tensor, enrollment: torch.Tensor) -> torch.Tensor:
        ...
        cond = self.cond(enrollment).view(batch, -1, 1, 1).expand(-1, -1, freq, frames)
        x1 = self.enc1(torch.cat([logmag, cond], dim=1))
...
def _stft(
    waves: torch.Tensor, *, n_fft: int, hop_length: int, window: torch.Tensor
) -> torch.Tensor:
    return torch.stft(
        waves,
        n_fft=n_fft,
...
def _train_model(
    ...
    *,
    ...
    for step in range(1, int(args.train_steps) + 1):
        mixture_np, target_np, interferer_np, enrollment_np = _synth_batch(
            targets,
            interferers,
            labels,
            centroids=centroids,
        ...
        if step == 1 or step % max(1, int(args.train_steps) // 10) == 0:
            print(
                ...
                f"mask={mask_loss.item():.5f} mag={mag_loss.item():.5f} wav={wav_loss.item():.5f}",
                flush=True,
            )
...
def _separate_batch(
    ...
    *,
    ...
    with torch.inference_mode():
        stft = _stft(mixture, n_fft=n_fft, hop_length=hop_length, window=window)
        mag = torch.abs(stft)
...
def _write_embeddings(
    ...
    *,
    ...
    for window_name, window_rows in sorted(rows_by_window.items()):
        ...
        print(f"embedded {window_name}: {len(window_rows)} unet-mask words", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train a one-hot conditioned STFT U-Net target-speaker mask."
    )
    parser.add_argument(
        "--prepared-root",
        type=Path,
    ...
    parser.add_argument("--cache-root", type=Path, default=Path("/tmp/codex_unet_mask_cache"))
    parser.add_argument(
        "--training-cache", type=Path, default=Path("/tmp/codex_real_stem_pairs_s60.npz")
    )
    parser.add_argument("--model-output", type=Path, default=Path("/tmp/codex_unet_mask.pt"))
    parser.add_argument(
        "--embedding-output", type=Path, default=Path("/tmp/codex_unet_mask_embeddings.npz")
    )
    parser.add_argument("--output", type=Path, default=Path("/tmp/codex_unet_mask_results.json"))
    parser.add_argument(
        "--dominance-json",
        type=Path,
    ...
    payload = {
        "model": "one_hot_stft_unet_mask_target_extractor",
    ...
    print(
        ",".join(
            [
                "unet_mask_true_target/lda_shrinkage",
