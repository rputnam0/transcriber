...
def _patch_speechbrain_yaml_loader() -> None:
    from ruamel import yaml

    for name in ("Loader", "RoundTripLoader", "SafeLoader"):
        cls = getattr(yaml, name, None)
        if cls is not None and not hasattr(cls, "max_depth"):
            setattr(cls, "max_depth", 0)


def _load_ecapa(*, device: str, savedir: Path):
    _patch_speechbrain_yaml_loader()
    ...
    *,
    ...
    for offset in range(0, len(waves), batch_size):
        ...
        with torch.inference_mode():
            embeddings = model.encode_batch(wave_batch)
        embedding_np = embeddings.detach().cpu().numpy().astype(np.float32).reshape(len(batch), -1)
        vectors.append(_normalize_rows(embedding_np))
        del wave_batch
        del embeddings
    ...
    *,
    ...
    for window_name, window_rows in sorted(_rows_by_window(rows).items()):
        ...
        all_embeddings.append(
            _embed_waveforms_ecapa(model, waves, batch_size=batch_size, device=device)
        )
...
def _write_clean_embeddings(
    *,
    ...
    embeddings = _embed_waveforms_ecapa(model, list(targets), batch_size=batch_size, device=device)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(
        output_path,
...
def _evaluate(
    ...
    *,
    ...
    for model_name in model_names:
        predictions[f"ecapa_clean_plus_mixed_lgo/{model_name}"] = ["unknown"] * len(rows)
    ...
    for group in groups:
        ...
        for model_name in model_names:
            ...
            labels = predictions[f"ecapa_clean_plus_mixed_lgo/{model_name}"]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Evaluate SpeechBrain ECAPA speaker embeddings on the flattened speaker-ID rows."
    )
    parser.add_argument(
        "--prepared-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/tmp/codex_speechbrain_ecapa_embedding_sweep.json"),
    )
    parser.add_argument(
        "--titanet-cache-root",
        type=Path,
    ...
    parser.add_argument("--savedir", type=Path, default=Path("/tmp/codex_speechbrain_ecapa"))
    ...
    if (
        ...
        model = _load_ecapa(device=device, savedir=args.savedir.expanduser())
    ...
    payload = {
        "model": "speechbrain_ecapa_embedding_sweep",
